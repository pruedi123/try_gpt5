...
if people and st.session_state.get("do_calc"):
    ...
    if max_years > 0:
        ...
        # Build a display-only copy with currency columns formatted as strings
        currency_keys = ["Own", "Spousal", "Total SS", "OwnCalc", "Monthly@Claim", "Household Total SS"]
        display_ss = ss_df.copy()
        # Remove "MonthsPaidOwn" column if present
        if "MonthsPaidOwn" in display_ss.columns:
            display_ss = display_ss.drop(columns=["MonthsPaidOwn"])
