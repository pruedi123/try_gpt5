...
#
# ═══════════════════════════════════════════════════════════════════════
# SECTION: Social Security Math Helpers (FRA, reductions, DRCs)
# ═══════════════════════════════════════════════════════════════════════
...
#
# ═══════════════════════════════════════════════════════════════════════
# SECTION: Month Arithmetic Helpers (first-of-month, add-months)
# ═══════════════════════════════════════════════════════════════════════
...
#
# ═══════════════════════════════════════════════════════════════════════
# SECTION: Additional Age & Survivor Utilities
# ═══════════════════════════════════════════════════════════════════════
...
def survivor_annual(survivor: 'Person', decedent: 'Person', death_date: date) -> float:
    """Survivor benefit payable annually, using death_date as the decedent's actual date of death.
    Rules simplified but realistic:
    - Base is decedent's benefit at death:
        * If decedent had already claimed before death → base = benefit at claim (no further DRC accrual after claim)
        * If death occurred after FRA and before claiming → base = PIA with delayed credits up to death
        * If death occurred before FRA and before claiming → base ≈ PIA (no early reduction)
    - Apply survivor’s early-claim reduction to that base.
    - Cap: survivor benefit cannot exceed decedent's benefit at death (RIB-LIM upper bound proxy).
      (Floor such as 82.5% of PIA is omitted for now; can be added if needed.)
    - Eligibility: survivor age >= 60 (ignores child-in-care exceptions).
    """
    # Eligibility check: survivor must be at least 60
    surv_age_y, surv_age_m = age_ym(survivor.dob, death_date)
    if (surv_age_y * 12 + surv_age_m) < 60 * 12:
        return 0.0

    # Decedent's benefit at death
    # Determine if decedent had claimed by death
    claimed_by_death = death_date >= decedent.claim_date
    if claimed_by_death:
        # Benefit fixed at claim level
        ...
        dec_benefit_at_death_mo = retirement_adjustment(decedent.pia_at_fra, months_from_fra_at_claim)
    else:
        # Not claimed at death
        fra_y, fra_m = decedent.fra
        # months from FRA at death; positive => delayed credits, negative => before FRA
        age_y, age_m = age_ym(decedent.dob, death_date)
        months_from_fra_at_death = months_between(age_y, age_m, fra_y, fra_m)
        if months_from_fra_at_death >= 0:
            # delayed credits up to death (cap at 70)
            cap_months = min(months_from_fra_at_death, (70 - fra_y) * 12)
            dec_benefit_at_death_mo = retirement_adjustment(decedent.pia_at_fra, cap_months)
        else:
            # death before FRA and before claiming → approximate with PIA
            dec_benefit_at_death_mo = decedent.pia_at_fra

    # Survivor's early-claim reduction relative to survivor's FRA at the month benefits begin (use death_date as anchor)
    surv_age_y, surv_age_m = age_ym(survivor.dob, death_date)
    ...
    # Apply reduction to decedent's base and cap to decedent's benefit at death (RIB-LIM upper bound)
    survivor_monthly = min(dec_benefit_at_death_mo, dec_benefit_at_death_mo * factor)
    return 12.0 * survivor_monthly

#
# ═══════════════════════════════════════════════════════════════════════
# SECTION: Person Dataclass & Derived Properties
# ═══════════════════════════════════════════════════════════════════════
@dataclass
class Person:
    name: str
...
if people:
    ...
    if len(people) >= 1:
        ...
        for y in future_years:
            ...
            row = {"Year": y}
            # ── OWN Benefits (prorated; couple capped at spouse death month) ───────
            ...
            # Track months paid for debug columns
            ...
            # ── SPOUSAL Supplements (both alive & both filed) ──────────────────────
            ...
            # ── SURVIVOR Replacement (higher-of own vs survivor) ───────────────────
            ...
            if len(people) == 2:
                ...
                def add_survivor_months(surv: Person, dec: Person, dec_death_first: date):
                    ...
                    if dec_death_first <= year_start:
                        ...
                        if months_surv > 0:
                            surv_mo_amt = survivor_annual(surv, dec, dec_death_first) / 12.0
            ...
            # ── Output Row Assembly (with Debug Columns) ───────────────────────────
