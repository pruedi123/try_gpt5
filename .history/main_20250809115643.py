...
if people and st.session_state.get("do_calc"):
    ...
    st.dataframe(
        ...
        column_config={
            "PIA @ FRA (monthly)": st.column_config.NumberColumn(format="$%,d"),
        }
    )
    ...
    if max_years > 0:
        ...
        # Build a column_config that formats likely currency fields without decimals, but keeps underlying floats
        ...
        for col in ss_df.columns:
            if any(k in col for k in currency_keys):
                colconf[col] = st.column_config.NumberColumn(format="$%,d")
