...
def safe_eol_date(dob: date, life_age: int) -> date:
    """End-of-life date as the birthday on which they turn `life_age`.
    Handles Feb 29 gracefully by using Feb 28 in non-leap years."""
    year = dob.year + life_age
    try:
        return date(year, dob.month, dob.day)
    except ValueError:
        # Handle Feb 29 (or other invalid combos) by stepping back to the last valid day
        # Simple safe fallback: clamp day to 28
        day = min(dob.day, 28)
