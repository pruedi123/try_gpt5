...
if people and st.session_state.get("do_calc"):
    ...
    if max_years > 0:
        ...
        # Format numeric columns in ss_df to display as whole numbers (tolerate NA)
        num_cols = ss_df.select_dtypes(include=["number"]).columns
        for col in num_cols:
            ss_df[col] = pd.to_numeric(ss_df[col], errors="coerce").round(0).astype("Int64")
