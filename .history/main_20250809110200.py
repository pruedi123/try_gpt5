...
if people:
    ...
    if len(people) >= 1:
        ...
        for y in future_years:
            ...
            row = {"Year": y}
            # Own benefits (prorated by months in the year)
            own = {p.name: 0.0 for p in people}
            for p in people:
                # Payment window is from claim month to month before death; measured within this projection year
                monthly_amt = worker_monthly_at_claim(p)
                ...
                months_paid = clamp_months_in_year(claim_first, death_first, year_start, year_end_excl)
                own[p.name] = monthly_amt * months_paid
            ...
            # Survivor logic (prorated): if one dies during this year, survivor may get survivor benefit for remaining months
            if len(people) == 2:
                a, b = people[0], people[1]
                year_start = first_of_month(asof)
                year_end_excl = first_of_month(add_years_months(asof, 1, 0))
                ...
                # If B dies during this projection year and A is alive past that point
                if year_start < b_death <= year_end_excl:
                    # Survivor months for A are from B's death month to year end
                    surv_months = max(0, months_between_firsts(year_end_excl, b_death))
                    surv_mo_amt = survivor_annual(a, b, asof) / 12.0
                    # Replace A's post-death months with survivor amount by adding those months
                    total[a.name] += surv_mo_amt * surv_months
                # If A dies during this projection year and B survives
                if year_start < a_death <= year_end_excl:
                    surv_months = max(0, months_between_firsts(year_end_excl, a_death))
                    surv_mo_amt = survivor_annual(b, a, asof) / 12.0
                    total[b.name] += surv_mo_amt * surv_months
