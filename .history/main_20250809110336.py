        ...
        for y in future_years:
            ...
            if len(people) == 2:
                ...
                if year_start < b_death <= year_end_excl:
                    # Survivor months for A begin the month AFTER B's death, through year end (SSA: no payment for month of death)
                    surv_start = add_months(b_death, 1)
                    surv_months = max(0, months_between_firsts(year_end_excl, surv_start))
                ...
                if year_start < a_death <= year_end_excl:
                    surv_start = add_months(a_death, 1)
                    surv_months = max(0, months_between_firsts(year_end_excl, surv_start))
