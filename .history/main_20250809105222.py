...
if people:
    ...
    if len(people) >= 1:
        ...
        for y in future_years:
            ...
            alive = {p.name: (asof < eol_map[p.name]) for p in people}  # treat EOL birthday as deceased
            ...
            # Survivor logic: if one spouse is deceased and the other alive, 
            # allow the survivor to step up to survivor amount if higher than their current total.
            total = {p.name: own[p.name] + spousal[p.name] for p in people}
            if len(people) == 2:
                a, b = people[0], people[1]
                a_alive, b_alive = alive[a.name], alive[b.name]
                # If B deceased, A may receive survivor
                if a_alive and not b_alive:
                    surv_amt = survivor_annual(a, b, asof)
                    total[a.name] = max(total[a.name], surv_amt)
                # If A deceased, B may receive survivor
                if b_alive and not a_alive:
                    surv_amt = survivor_annual(b, a, asof)
                    total[b.name] = max(total[b.name], surv_amt)

            # Ensure no benefits are paid to anyone not alive
            for p in people:
                if not alive[p.name]:
                    own[p.name] = 0.0
                    spousal[p.name] = 0.0
            ...
            row["Household Total SS"] = round(sum(total[name] for name in total if alive[name]), 2)
            ss_rows.append(row)
