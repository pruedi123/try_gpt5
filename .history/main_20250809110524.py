...
if people:
    ...
    if len(people) >= 1:
        ...
        for y in future_years:
            ...
            # Survivor logic (prorated): if one dies during this year, survivor may get the HIGHER of own vs survivor
            if len(people) == 2:
                a, b = people[0], people[1]
                year_start = first_of_month(asof)
                year_end_excl = first_of_month(add_years_months(asof, 1, 0))
                ...
                # Helper to add survivor months for SURV, given DEC dies in this year
                def add_survivor_months(surv: Person, dec: Person, dec_death_first: date):
                    # Survivor months begin the month AFTER death and run to year end
                    ...
                    # We already stopped OWN at the spouse's death month above, so these are replacement months only
                    total[surv.name] += higher * months_surv

                # If B dies during this projection year and A survives past that point
