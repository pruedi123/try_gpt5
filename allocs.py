...
def aggregate_allocations_from_pdfs(pdf_files):
    ...
    for pdf_file in pdf_files:
        ...
        name_only = os.path.basename(pdf_file)
        if "AI Testing_" in name_only:
            name_only = name_only.split("AI Testing_")[-1]
        if name_only.lower().endswith(".pdf"):
            name_only = name_only[:-4]
        if total and equity is not None and fixed_income is not None and cash is not None and total != 0:
            pct_equity = round((equity / total) * 100.0)
            pct_fixed_incl_cash = round(((fixed_income + cash) / total) * 100.0)
        else:
            pct_equity = None
            pct_fixed_incl_cash = None
        results[name_only] = {"Equity": pct_equity, "Fixed": pct_fixed_incl_cash}
    return results
